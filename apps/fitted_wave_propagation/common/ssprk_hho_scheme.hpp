...
class ssprk_hho_scheme
{
    ...
    ssprk_hho_scheme(SparseMatrix<T> & Kg, Matrix<T, Dynamic, 1> & Fg, SparseMatrix<T> & Mg, size_t n_f_dof){
...
//        }
        ...
        // Cells update
        Matrix<T, Dynamic, 1> RHSc = Fc() - Kcc()*x_c_dof - Kcf()*x_f_dof;
        Matrix<T, Dynamic, 1> delta_x_c_dof = m_Mc_inv * RHSc;
        Matrix<T, Dynamic, 1> x_n_c_dof = a * x_c_dof + b * dt * delta_x_c_dof; // new state
        x_dof_n.block(0, 0, m_n_c_dof, 1) = x_n_c_dof;
    
        // Faces update
        Matrix<T, Dynamic, 1> RHSf = Kfc()*x_n_c_dof;
        if (m_sff_is_block_diagonal_Q) {
            x_dof_n.block(m_n_c_dof, 0, m_n_f_dof, 1) = - m_Sff_inv * RHSf;
        }else{
            if (m_iterative_solver_Q) {
                x_dof_n.block(m_n_c_dof, 0, m_n_f_dof, 1) = -m_analysis_cg.solve(RHSf); // new state
                std::cout << "Number of iterations (CG): " << m_analysis_cg.iterations() << std::endl;
                std::cout << "Estimated error: " << m_analysis_cg.error() << std::endl;
            }else{
                x_dof_n.block(m_n_c_dof, 0, m_n_f_dof, 1) = -FacesAnalysis().solve(RHSf); // new state
            }
        }
        
//        // Faces update
//        Matrix<T, Dynamic, 1> RHSf = Kfc()*x_n_c_dof;
//        Matrix<T, Dynamic, 1> x_n_f_dof = -FacesAnalysis().solve(RHSf); // new state
//    
...
//        x_dof_n.block(m_n_c_dof, 0, m_n_f_dof, 1) = x_n_f_dof;
    ...
    void ssprk_step(int s, Matrix<T, Dynamic, Dynamic> &alpha, Matrix<T, Dynamic, Dynamic> &beta, T & dt, Matrix<T, Dynamic, 1> & x_dof_n_m, Matrix<T, Dynamic, 1> & x_dof_n){
        
        size_t n_dof = x_dof_n_m.rows();
        Matrix<T, Dynamic, Dynamic> ys = Matrix<T, Dynamic, Dynamic>::Zero(n_dof, s+1);

        Matrix<T, Dynamic, 1> yn, ysi, yj;
        ys.block(0, 0, n_dof, 1) = x_dof_n_m;
        for (int i = 0; i < s; i++) {

            ysi = Matrix<T, Dynamic, 1>::Zero(n_dof, 1);
            for (int j = 0; j <= i; j++) {
                yn = ys.block(0, j, n_dof, 1);
                ssprk_weight(yn, yj, dt, alpha(i,j), beta(i,j));
                ysi += yj;
            }
