...
public:
  ...
  static void compute_errors_one_field(
			Mesh & msh,
			disk::hho_degree_info & hho_di,
			acoustic_one_field_assembler<Mesh> & assembler,
			Matrix<double, Dynamic, 1> & x_dof,
			std::function<double(const typename Mesh::point_type& )> scal_fun,
		        std::function<std::vector<double>(const typename Mesh::point_type& )> flux_fun,
  ...
  static void compute_errors_two_fields(
			Mesh & msh,
			disk::hho_degree_info & hho_di,
			acoustic_two_fields_assembler<Mesh> & assembler,
			Matrix<double, Dynamic, 1> & x_dof,
			std::function<double(const typename Mesh::point_type& )> scal_fun,
			std::function<std::vector<double>(const typename Mesh::point_type& )> flux_fun,
			std::ostream & error_file = std::cout, bool recons_error_Q = false) {
  ...
  static void write_silo_one_field(std::string silo_file_name,
				   size_t it, Mesh & msh, disk::hho_degree_info & hho_di,
				   Matrix<double, Dynamic, 1> & x_dof,
				   std::function<double(const typename Mesh::point_type& )> scal_fun,
				   bool cell_centered_Q) {
  ...
  static void write_silo_two_fields(
		       std::string silo_file_name, size_t it, Mesh & msh,
		       disk::hho_degree_info & hho_di, Matrix<double, Dynamic, 1> & x_dof,
		       std::function<double(const typename Mesh::point_type& )> scal_fun,
		       std::function<std::vector<double>(const typename Mesh::point_type& )> flux_fun,
		       bool cell_centered_Q) {
	...
	{
	  auto cell_basis = make_scalar_monomial_basis(msh, cell, hho_di.cell_degree());
	  Matrix<RealType, Dynamic, 1> scalar_cell_dof = x_dof.block(cell_i*cell_dof+n_vec_dof,
								     0, n_scal_dof, 1);
	  auto t_phi = cell_basis.eval_functions( bar );
	  RealType uh = scalar_cell_dof.dot( t_phi );
	  approx_u.push_back(uh);
  ...
  static void compute_errors_one_field_vectorial(Mesh & msh,
	       disk::hho_degree_info & hho_di,
	       elastodynamic_one_field_assembler<Mesh> & assembler,
	       Matrix<double, Dynamic, 1> & x_dof,
               std::function<static_vector<double, 2>(const typename Mesh::point_type& )> vec_fun,
	       std::function<static_matrix<double, 2, 2>(const typename Mesh::point_type& )> flux_fun,
	       std::ostream & error_file = std::cout) {
	...
	Matrix<RealType, Dynamic, Dynamic> mass = make_mass_matrix(msh, cell, cell_basis,
								   hho_di.cell_degree());
	...
	for (auto & point_pair : int_rule) {
	  ...
	  auto sigma = 2.0 * mu * epsilon
	                        + lambda * trace_epsilon * static_matrix<RealType, 2, 2>::Identity();
    ...
    std::cout << bold << cyan << "Error completed: " << tc << " seconds" << reset << std::endl;
    error_file << "Characteristic h size = " << std::setprecision(16) << h << std::endl;
    error_file << "L2-norm error = " << std::setprecision(16) << std::sqrt(vector_l2_error)
	                                                      << std::endl;
    error_file << "H1-norm error = " << std::setprecision(16) << std::sqrt(flux_l2_error) << std::endl;
    error_file << std::endl;
    error_file.flush();  
    ...
    static void compute_errors_two_fields_vectorial(
	       Mesh & msh,
	       disk::hho_degree_info & hho_di, Matrix<double, Dynamic, 1> & x_dof,
	       std::function<static_vector<double, 2>(const typename Mesh::point_type& )> vec_fun,
	       std::function<static_matrix<double, 2, 2>(const typename Mesh::point_type& )> flux_fun,
	...
	Matrix<RealType, Dynamic, 1> vec_cell_dof = x_dof.block(cell_i*cell_dof+n_ten_cbs, 0,
								n_vec_cbs, 1);
	...
	cell_i++;
      ...
      error_file << "L2-norm error = " << std::setprecision(16) << std::sqrt(vector_l2_error)
		                                                << std::endl;
      error_file << "H1-norm error = " << std::setprecision(16) << std::sqrt(flux_l2_error)
		                                                << std::endl;
      error_file << std::endl;
      error_file.flush();
  ...
  static void compute_errors_three_fields_vectorial(
	       Mesh & msh,
	       disk::hho_degree_info & hho_di, Matrix<double, Dynamic, 1> & x_dof,
	       std::function<static_vector<double, 2>(const typename Mesh::point_type& )> vec_fun,
	       std::function<static_matrix<double, 2, 2>(const typename Mesh::point_type& )> flux_fun,
	...
	h = h_l;
      ...
      Matrix<RealType, Dynamic, 1> vec_cell_dof = x_dof.block(cell_i*cell_dof+n_ten_cbs+n_sca_cbs,
							      0, n_vec_cbs, 1);
      
      // vector evaluation
      {
	auto cell_basis = disk::make_vector_monomial_basis(msh, cell, hho_di.cell_degree());
	Matrix<RealType, Dynamic, Dynamic> mass = make_mass_matrix(msh, cell,
								   cell_basis, hho_di.cell_degree());
	Matrix<RealType, Dynamic, 1> rhs = make_rhs(msh, cell, cell_basis, vec_fun);
	Matrix<RealType, Dynamic, 1> real_dofs = mass.llt().solve(rhs);
	Matrix<RealType, Dynamic, 1> diff = real_dofs - vec_cell_dof;
	vector_l2_error += diff.dot(mass*diff);	
	...
	Matrix<RealType, Dynamic, 1> sigma_v_x_cell_dof = x_dof.block(cell_i*cell_dof+n_ten_cbs,
								      0, n_sca_cbs, 1);
  ...
  static void write_silo_one_field_vectorial(
  ...
  static void write_silo_two_fields_vectorial(
	...
	{
	  auto cell_basis = make_vector_monomial_basis(msh, cell, hho_di.cell_degree());
	  Matrix<RealType, Dynamic, 1> vec_x_cell_dof = x_dof.block(cell_i*cell_dof + n_ten_cbs, 0,
								    n_vec_cbs, 1);
	  auto t_phi = cell_basis.eval_functions( bar );
	  assert(t_phi.rows() == cell_basis.size());
	  auto uh = disk::eval(vec_x_cell_dof, t_phi);
	...
	{
	  auto cell_basis = make_vector_monomial_basis(msh, cell, hho_di.cell_degree());
	  Matrix<RealType, Dynamic, 1> vec_x_cell_dof = x_dof.block(cell_i*cell_dof + n_ten_cbs,
								    0, n_vec_cbs, 1);
	  auto t_phi = cell_basis.eval_functions( bar );
	  assert(t_phi.rows() == cell_basis.size());
	  auto uh = disk::eval(vec_x_cell_dof, t_phi);
  ...
  static void write_silo_three_fields_vectorial(std::string silo_file_name,
	...
	// tensor evaluation
	{
	  ...
	  auto sca_basis = disk::make_scalar_monomial_basis(msh, cell, hho_di.face_degree());
	  Matrix<RealType, Dynamic, 1> sigma_v_x_cell_dof = x_dof.block(cell_i*cell_dof+n_ten_cbs, 0,
									n_sca_cbs, 1);
  ...
  static void write_silo_acoustic_property_map(
					   std::string silo_file_name,
					   Mesh & msh,
					   std::vector< acoustic_material_data<double> > & material) {
    ...
    tc.toc();
    std::cout << std::endl;
    std::cout << bold << cyan << "Properties file rendered in : " << tc << " seconds" << reset
	                                                                              << std::endl;
  }
  
  /// Compute the discrete acoustic energy for one field approximation
  static double compute_acoustic_energy_one_field(Mesh & msh, disk::hho_degree_info & hho_di,
						  acoustic_one_field_assembler<Mesh> & assembler,
						  double & time, Matrix<double, Dynamic, 1> & p_dof,
						  Matrix<double, Dynamic, 1> & v_dof,
						  std::ostream & energy_file = std::cout) {
  ...
  /// Compute the discrete acoustic energy for one field approximation
  static double compute_acoustic_energy_two_fields(Mesh & msh, disk::hho_degree_info & hho_di,
						   acoustic_two_fields_assembler<Mesh> & assembler,
						   double & time, Matrix<double, Dynamic, 1> & x_dof,
						   std::ostream & energy_file = std::cout) {
  ...
  static double compute_elastic_energy_one_field(Mesh & msh, disk::hho_degree_info & hho_di,
						 elastodynamic_one_field_assembler<Mesh> & assembler,
						 double & time, Matrix<double, Dynamic, 1> & u_dof,
						 Matrix<double, Dynamic, 1> & v_dof,
						 std::ostream & energy_file = std::cout) {
    ...
    tbb::parallel_for(size_t(0), size_t(n_cells), size_t(1),
      ...
      Matrix<RealType, Dynamic, Dynamic> laplacian_loc = assembler.laplacian_operator(cell_ind, msh,
										      cell);
  ...
  static double compute_elastic_energy_two_fields(Mesh & msh, disk::hho_degree_info & hho_di,
						  elastodynamic_two_fields_assembler<Mesh> & assembler,
						  double & time, Matrix<double, Dynamic, 1> & x_dof,
						  std::ostream & energy_file = std::cout){
    ...
    using RealType = double;
    size_t n_ten_cbs = disk::sym_matrix_basis_size(hho_di.grad_degree(), Mesh::dimension,
						   Mesh::dimension);
    ...
    tbb::parallel_for(size_t(0), size_t(n_cells), size_t(1),
      ...
      Matrix<RealType, Dynamic, Dynamic> mass_matrix_v = mass_matrix.block(n_ten_cbs, n_ten_cbs,
									   n_vec_cbs, n_vec_cbs);
      ...
      Matrix<RealType, Dynamic, Dynamic> mass_matrix_stress = mass_matrix.block(0, 0, n_ten_cbs,
										n_ten_cbs);
  ...
  static double compute_elastic_energy_three_fields(
					       Mesh & msh, disk::hho_degree_info & hho_di,
					       elastodynamic_three_fields_assembler<Mesh> & assembler,
					       double & time, Matrix<double, Dynamic, 1> & x_dof,
					       std::ostream & energy_file = std::cout) {
    ...
    tbb::parallel_for(size_t(0), size_t(n_cells), size_t(1),
     ...
     Matrix<RealType, Dynamic, Dynamic> mass_matrix_v = mass_matrix.block(n_ten_cbs + n_sca_cbs,
									  n_ten_cbs + n_sca_cbs,
									  n_vec_cbs, n_vec_cbs);
  ...
  static void write_silo_elastic_property_map(std::string silo_file_name, Mesh & msh,
					      std::vector< elastic_material_data<double> > & material){
  ...
  static void write_silo_two_fields_elastoacoustic(
					 std::string silo_file_name, size_t it, Mesh & msh,
					 disk::hho_degree_info & hho_di,
					 Matrix<double, Dynamic, 1> & x_dof,
					 std::map<size_t,elastic_material_data<double>> & e_material,
					 std::map<size_t,acoustic_material_data<double>> & a_material,
					 bool cell_centered_Q){
	...
	{
	  auto cell_basis = make_vector_monomial_basis(msh, cell, hho_di.cell_degree());
	  Matrix<RealType, Dynamic, 1> vec_x_cell_dof = x_dof.block(e_cell_ind*e_cell_dof, 0,
								    e_cell_dof, 1);
	  auto t_phi = cell_basis.eval_functions( bar );
	  assert(t_phi.rows() == cell_basis.size());
	  auto uh = disk::eval(vec_x_cell_dof, t_phi);
	...
	// scalar evaluation
	{
	  ...
	  Matrix<RealType, Dynamic, 1> scalar_cell_dof = x_dof.block(a_cell_ind*a_cell_dof +
								     n_elastic_cell_dof, 0, a_cell_dof,
								     1);
	  auto t_phi = cell_basis.eval_functions( bar );
	  RealType uh = scalar_cell_dof.dot( t_phi );
  ...
  static void write_silo_four_fields_elastoacoustic(
					 std::string silo_file_name,
					 size_t it, Mesh & msh,
					 disk::hho_degree_info & hho_di,
					 Matrix<double, Dynamic, 1> & x_dof,
	...
	{
	  auto cell_basis = make_vector_monomial_basis(msh, cell, hho_di.cell_degree());
	  Matrix<RealType, Dynamic, 1> vec_x_cell_dof = x_dof.block(e_cell_ind*e_cell_dof+n_ten_cbs, 0,
								    n_vec_cbs, 1);
	  auto t_phi = cell_basis.eval_functions( bar );
	  assert(t_phi.rows() == cell_basis.size());
	  auto uh = disk::eval(vec_x_cell_dof, t_phi);
  ...
  static void compute_errors_two_fields_elastoacoustic(
	      Mesh & msh,
	      disk::hho_degree_info & hho_di,
	      elastoacoustic_two_fields_assembler<Mesh> & assembler,
	      Matrix<double, Dynamic, 1> & x_dof,
	      std::function<static_vector<double, 2>(const typename Mesh::point_type& )> vec_fun,
	      std::function<static_matrix<double, 2, 2>(const typename Mesh::point_type& )> sigma_fun,
	      std::function<double(const typename Mesh::point_type& )> scal_fun,
	      std::function<static_vector<double, 2>(const typename Mesh::point_type& )> flux_fun,
	      std::ostream & error_file = std::cout){
	...
	vector_l2_error += diff.dot(mass*diff);
      ...
      {
	auto int_rule = integrate(msh, cell, 2*(hho_di.cell_degree()+1));
	Matrix<RealType, Dynamic, 1> all_dofs = assembler.gather_e_dof_data(e_cell_ind, msh,
									    cell, x_dof);
	...
	h = h_l;
      ...
      Matrix<RealType, Dynamic, 1> scalar_cell_dof = x_dof.block(a_cell_ind*a_cell_dof +
								 n_elastic_cell_dof, 0, a_cell_dof, 1);
	...
	auto rec_basis = disk::make_scalar_monomial_basis(msh, cell, hho_di.reconstruction_degree());
	auto gr = make_scalar_hho_laplacian(msh, cell, hho_di);
	Matrix<RealType, Dynamic, 1> all_dofs = assembler.gather_a_dof_data(a_cell_ind, msh,
									    cell, x_dof);
	Matrix<RealType, Dynamic, 1> recdofs = gr.first * all_dofs;
    ...
    error_file << "L2-norm error = " << std::setprecision(16) << std::sqrt(vector_l2_error)
	                                                      << std::endl;
    error_file << "H1-norm error = " << std::setprecision(16) << std::sqrt(sigma_l2_error)
	                                                      << std::endl;
    error_file << "Acoustic region : " << std::endl;
    error_file << "L2-norm error = " << std::setprecision(16) << std::sqrt(scalar_l2_error)
	                                                      << std::endl;
    error_file << "H1-norm error = " << std::setprecision(16) << std::sqrt(flux_l2_error) << std::endl;
    error_file << std::endl;
    error_file.flush();
  ...
  static void compute_errors_four_fields_elastoacoustic(
	      Mesh & msh,
	      disk::hho_degree_info & hho_di,
	      elastoacoustic_four_fields_assembler<Mesh> & assembler,
	      Matrix<double, Dynamic, 1> & x_dof,
	      std::function<static_vector<double, 2>(const typename Mesh::point_type& )> vec_fun,
	      std::function<static_matrix<double, 2, 2>(const typename Mesh::point_type& )> sigma_fun,
	      std::function<double(const typename Mesh::point_type& )> scal_fun,
    ...
    size_t n_ten_cbs = disk::sym_matrix_basis_size(hho_di.grad_degree(),
						   Mesh::dimension,
						   Mesh::dimension);
    
    size_t n_vec_cbs = disk::vector_basis_size(hho_di.cell_degree(),
					       Mesh::dimension,
					       Mesh::dimension);
    
    size_t e_cell_dof = n_ten_cbs + n_vec_cbs;
    
    size_t n_vel_scal_cbs = disk::scalar_basis_size(hho_di.reconstruction_degree(),
						    Mesh::dimension) - 1;
	...
	h = h_l;
      ...
      Matrix<RealType, Dynamic, 1> vec_cell_dof = x_dof.block(e_cell_ind*e_cell_dof + n_ten_cbs, 0,
							      n_vec_cbs, 1);
	...
	auto int_rule = integrate(msh, cell, 2*(hho_di.cell_degree()+1));
	auto ten_basis = make_sym_matrix_monomial_basis(msh, cell, hho_di.grad_degree());
	Matrix<RealType, Dynamic, 1> ten_x_cell_dof = x_dof.block(e_cell_ind*e_cell_dof, 0,
								  n_ten_cbs, 1);
	// Error integrals
	for (auto & point_pair : int_rule) {
	  RealType omega = point_pair.weight();
	...
	h = h_l;
      ...
      Matrix<RealType, Dynamic, 1> scalar_cell_dof = x_dof.block(a_cell_ind*a_cell_dof +
								 n_elastic_cell_dof+n_vel_scal_cbs, 0,
								 n_scal_cbs, 1);
	...
	auto int_rule = integrate(msh, cell, 2*(hho_di.cell_degree()+1));
	auto cell_basis = make_scalar_monomial_basis(msh, cell, hho_di.reconstruction_degree());
	Matrix<RealType, Dynamic, 1> flux_cell_dof = x_dof.block(a_cell_ind*a_cell_dof +
								 n_elastic_cell_dof, 0,
								 n_vel_scal_cbs, 1);
	// Error integrals
	for (auto & point_pair : int_rule) {
  ...
  static std::set<size_t> find_cells(typename Mesh::point_type & pt, Mesh & msh,
				     bool verbose_Q = false) {
        
    using RealType = double;
    auto norm =  [](const typename Mesh::point_type& a, const typename Mesh::point_type& b )
                                                                                       -> RealType {
      RealType dx = (b.x() - a.x());
      RealType dy = (b.y() - a.y());
  ...
  static size_t pick_cell(typename Mesh::point_type & pt, Mesh & msh,
			  std::set<size_t> & cell_indexes, bool verbose_Q = false) {
    
    using RealType = double;
    
    auto triangle_member_Q = [] (typename Mesh::point_type & p, typename Mesh::point_type & p0,
				 typename Mesh::point_type & p1, typename Mesh::point_type & p2)
    {
  ...
  static void record_data_acoustic_one_field(
			      size_t it,
			      std::pair<typename Mesh::point_type,size_t> & pt_cell_index, Mesh & msh,
			      disk::hho_degree_info & hho_di, Matrix<double, Dynamic, 1> & x_dof,
			      std::ostream & seismogram_file = std::cout){
  ...
  static void record_velocity_data_acoustic_one_field(
				size_t it,
				std::pair<typename Mesh::point_type,size_t> & pt_cell_index,
				Mesh & msh, disk::hho_degree_info & hho_di,
				acoustic_one_field_assembler<Mesh> & assembler,
				Matrix<double, Dynamic, 1> & x_dof,
				std::ostream & seismogram_file = std::cout){
    ...
    tc.toc();
    std::cout << bold << cyan << "Value recorded: " << tc << " seconds" << reset << std::endl;
    seismogram_file << it << "," << std::setprecision(16) <<  vh(0,0) << "," << std::setprecision(16)
		    <<  vh(1,0) << std::endl;
    seismogram_file.flush();
  ...
  static void record_data_acoustic_two_fields(
			    size_t it,
			    std::pair<typename Mesh::point_type,size_t> & pt_cell_index, Mesh & msh,
			    disk::hho_degree_info & hho_di, Matrix<double, Dynamic, 1> & x_dof,
			    std::ostream & seismogram_file = std::cout){
  ...
  static void record_velocity_data_acoustic_two_fields(
					 size_t it,
					 std::pair<typename Mesh::point_type,size_t> & pt_cell_index,
					 Mesh & msh, disk::hho_degree_info & hho_di,
					 Matrix<double, Dynamic, 1> & x_dof,
  ...
  static void record_data_elastic_one_field(
				     size_t it,
				     std::pair<typename Mesh::point_type,size_t> & pt_cell_index,
				     Mesh & msh, disk::hho_degree_info & hho_di,
				     Matrix<double, Dynamic, 1> & x_dof,
  ...
  static void record_data_elastic_two_fields(
			  size_t it,
			  std::pair<typename Mesh::point_type,size_t> & pt_cell_index,
			  Mesh & msh,
			  disk::hho_degree_info & hho_di, Matrix<double, Dynamic, 1> & x_dof,
	...
	auto& cell = msh.backend_storage()->surfaces[cell_ind];
	auto cell_basis = make_vector_monomial_basis(msh, cell, hho_di.cell_degree());
	Matrix<RealType, Dynamic, 1> vec_x_cell_dof = x_dof.block(cell_ind*cell_dof + n_ten_cbs ,
								  0, n_vec_cbs, 1);
	auto t_phi = cell_basis.eval_functions( pt );
	assert(t_phi.rows() == cell_basis.size());
	vh = disk::eval(vec_x_cell_dof, t_phi);
  ...
  static void record_data_elastic_three_fields(
			      size_t it,
			      std::pair<typename Mesh::point_type,size_t> & pt_cell_index,
			      Mesh & msh, disk::hho_degree_info & hho_di,
			      Matrix<double, Dynamic, 1> & x_dof,
  ...
  static void record_velocity_data_elasto_acoustic_two_fields(
				  size_t it,
				  std::pair<typename Mesh::point_type,size_t> & pt_cell_index,
				  Mesh & msh, disk::hho_degree_info & hho_di,
				  elastoacoustic_two_fields_assembler<Mesh> & assembler,
				  Matrix<double, Dynamic, 1> & u_dof,
				  Matrix<double, Dynamic, 1> & v_dof, bool e_side_Q,
				  std::ostream & seismogram_file = std::cout){
	...
	auto& cell = msh.backend_storage()->surfaces[cell_ind];
	auto cell_basis = make_vector_monomial_basis(msh, cell, hho_di.cell_degree());
	Matrix<RealType, Dynamic, 1> all_dofs = assembler.gather_e_dof_data(cell_ind, msh,
									    cell, v_dof);
	...
	vh = disk::eval(vec_x_cell_dof, t_phi);
      ...
      else { // reconstructed velocity evaluation
	
	size_t cell_dof = disk::scalar_basis_size(hho_di.cell_degree(), dim);
	auto& cell = msh.backend_storage()->surfaces[cell_ind];
	auto rec_basis = disk::make_scalar_monomial_basis(msh, cell, hho_di.reconstruction_degree());
	auto gr = make_scalar_hho_laplacian(msh, cell, hho_di);
	Matrix<RealType, Dynamic, 1> all_dofs = assembler.gather_a_dof_data(cell_ind, msh,
									    cell, u_dof);
	Matrix<RealType, Dynamic, 1> recdofs = gr.first * all_dofs;
	...
	acoustic_material_data<RealType> a_mat = assembler.get_a_material_data().find(cell_ind)
	                                                                                   -> second;
  ...
  static void record_velocity_data_elasto_acoustic_four_fields(
			  size_t it,
			  std::pair<typename Mesh::point_type,size_t> & pt_cell_index, Mesh & msh,
			  disk::hho_degree_info & hho_di,
			  elastoacoustic_four_fields_assembler<Mesh> & assembler,
			  Matrix<double, Dynamic, 1> & x_dof,
			  bool e_side_Q, std::ostream & seismogram_file = std::cout){
    ...
    size_t n_ten_cbs = disk::sym_matrix_basis_size(hho_di.grad_degree(), Mesh::dimension,
						   Mesh::dimension);
    size_t n_vec_cbs = disk::vector_basis_size(hho_di.cell_degree(),Mesh::dimension, 
    					       Mesh::dimension);
    ...
    tc.toc();
    std::cout << bold << cyan << "      Sensor : Value recorded in " << tc << " seconds" << reset
	                                                                   << std::endl;
    seismogram_file << it << "," << std::setprecision(16) <<  vh(0,0) << "," << std::setprecision(16)
		    <<  vh(1,0) << std::endl;
    seismogram_file.flush();
