...
public:
  ...
  (const typename disk::generic_mesh<double, 2>::point_type& )> Evaluate_v(double & t){
	...
	x = pt.x();
	y = pt.y();
	vx = - (std::sqrt(2.0)*M_PI*x*x*std::cos((M_PI*x)/2.)*
		                                    std::sin(std::sqrt(2.0)*M_PI*t)*std::sin(M_PI*y));
	vy = - (std::sqrt(2.0)*M_PI*x*x*std::cos((M_PI*x)/2.)*
		                                    std::sin(std::sqrt(2.0)*M_PI*t)*std::sin(M_PI*y));
  ...
  (const typename disk::generic_mesh<double, 2>::point_type& )> Evaluate_f(double & t){
    ...
    case EFunctionNonPolynomial: {
     ...
     x = pt.x();
     y = pt.y();
     fx = - ( std::cos(std::sqrt(2.0)*M_PI*t) *
	      ( -4*M_PI*x*std::sin((M_PI*x)/2.) * ( M_PI*x*std::cos(M_PI*y) + 6*std::sin(M_PI*y) )
	      + std::cos((M_PI*x)/2.) * ( 16*M_PI*x*std::cos(M_PI*y) + (24 + M_PI*M_PI*x*x)
					                                    * std::sin(M_PI*y))))/4.0;
     fy =   ( std::cos(std::sqrt(2.0)*M_PI*t) *
	    ( 4*M_PI*x*std::sin((M_PI*x)/2.)  * (M_PI*x*std::cos(M_PI*y) + 2*std::sin(M_PI*y) )
	      + std::cos((M_PI*x)/2.)*(-16*M_PI*x*std::cos(M_PI*y) + (-8 + 5*M_PI*M_PI*x*x)
				                                              *std::sin(M_PI*y))))/4.;
  ...
  (const typename disk::generic_mesh<double, 2>::point_type& )> Evaluate_sigma(double & t){
	...
	sigma(0,0) = M_PI*x*x*std::cos(std::sqrt(2.0)*M_PI*t)*std::cos((M_PI*x)/2.)*std::cos(M_PI*y)
	             + 4*x*std::cos(std::sqrt(2.0)*M_PI*t)*std::cos((M_PI*x)/2.)*std::sin(M_PI*y)
	             - M_PI*x*x*std::cos(std::sqrt(2.0)*M_PI*t)*std::sin((M_PI*x)/2.)*std::sin(M_PI*y)
	             + ( 4*x*std::cos(std::sqrt(2.0)*M_PI*t)*std::cos((M_PI*x)/2.)*std::sin(M_PI*y)
			 - M_PI*x*x*std::cos(std::sqrt(2.0)*M_PI*t)*std::sin((M_PI*x)/2.)
			                                                        *std::sin(M_PI*y))/2.0;
        
	sigma(0,1) = M_PI*x*x*std::cos(std::sqrt(2.0)*M_PI*t)*std::cos((M_PI*x)/2.)*std::cos(M_PI*y)
	             + 2*x*std::cos(std::sqrt(2.0)*M_PI*t)*std::cos((M_PI*x)/2.)*std::sin(M_PI*y)
	             - (M_PI*x*x*std::cos(std::sqrt(2.0)*M_PI*t)*std::sin((M_PI*x)/2.)
			                                                         *std::sin(M_PI*y))/2.;
  ...
  (const typename disk::generic_mesh<double, 2>::point_type& )> Evaluate_s_v(double & t){
	...
	x = pt.x();
	y = pt.y();
	return std::sqrt(2.0)*M_PI*x*x*std::cos(std::sqrt(2.0)*M_PI*t)
  ...
  (const typename disk::generic_mesh<double, 2>::point_type& )> Evaluate_s_f(double & t){
	...
	x = pt.x();
	y = pt.y();
	f = -2*std::sin(std::sqrt(2.0)*M_PI*t)*(2*M_PI*x*std::cos(M_PI*x)
						                 + std::sin(M_PI*x))*std::sin(M_PI*y);
	return f;
      };
    }
  ...
  (const typename disk::generic_mesh<double, 2>::point_type& )> Evaluate_s_q(double & t){
	...
	x = pt.x();
	y = pt.y();
	qx = M_PI*x*x*std::cos(M_PI*x)*std::sin(std::sqrt(2.0)*M_PI*t)*std::sin(M_PI*y)
	                      + 2*x*std::sin(std::sqrt(2.0)*M_PI*t)*std::sin(M_PI*x)*std::sin(M_PI*y);
