...
public:
  ...
  simulation_data() : m_k_degree(2), m_n_divs(2), m_hdg_stabilization_Q(false),
		      ...
		      m_iterative_solver_Q(false),  m_polygonal_mesh_Q(false), m_exact_functions(1) {
  ...
  void print_simulation_data(){
    ...
    std::cout << yellow;
    std::cout << bold << "         - Refinement level     -l : " << m_n_divs << std::endl;
    ...
    std::cout << bold << cyan << std::endl;
    std::cout << "      ";        
    std::cout << "Exact function : " << m_exact_functions << reset << std::endl;
