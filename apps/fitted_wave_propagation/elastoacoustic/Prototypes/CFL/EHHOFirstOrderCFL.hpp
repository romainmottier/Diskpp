...
void EHHOFirstOrderCFL(int argc, char **argv){
    ...
    std::cout << std::endl << bold << red << "   COUPLING CFL" << std::endl << std::endl;
    ...
    for (auto face_it = msh.faces_begin(); face_it != msh.faces_end(); face_it++) {
        ...
        for (int s = 2; s < 5; s++) {    
            ...
            filename << "E_explicit_l_" << sim_data.m_n_divs << "_n_" << sim_data.m_nt_divs << "_k_" << sim_data.m_k_degree << "_s_" << s << "_discret_" << sim_data.m_hdg_stabilization_Q << ".txt";
            ...
            for (unsigned int i = 0; i < 1000; i++) {
                ...
                energy_0 = postprocessor<mesh_type>::compute_elasto_acoustic_energy_four_field(msh, hho_di, assembler, t, x_dof, energy_file);
                ...
                for(size_t it = 1; it <= nt; it++) {
                    ...
                    bool unstable_check_Q = (relative_energy_0 >= 5e-2);
                    // bool unstable_check_Q = (relative_energy > 15e-2) || (relative_energy_0 >= 15e-2);
