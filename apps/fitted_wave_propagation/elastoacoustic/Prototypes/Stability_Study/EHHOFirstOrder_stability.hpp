...
// ../../../../elastoacoustic -k 1 -s 1 -r 0 -c 1 -p 0 -l 0 -n 0 -f 0 -e 0 

void EHHOFirstOrder_stability(int argc, char **argv);
...
void EHHOFirstOrder_stability(int argc, char **argv){
    ...
    std::cout << std::endl << bold << red << "   COUPLING STABILITY" << reset << std::endl;
    ...
    if (sim_data.m_polygonal_mesh_Q) {
        ...
        // mesh_files.push_back("../../../../../meshes/conv_test/poly/poly_32768.txt");  // -l 10
    ...
    if (sim_data.m_render_silo_files_Q) {
        ...
        postprocessor<mesh_type>::write_silo_four_fields_elastoacoustic(silo_str, 0, msh, hho_di, x_dof, e_material, a_material, false);      
    }
    ...
    for(size_t it = 1; it <= nt; it++) {
        ...
        tcit.tic();
        std::cout << bold << red << "   Time step number " << it << ": t = " << t << reset;
        ...
        bool unstable_check_Q = (relative_energy > 5.0e-2) || (relative_energy_0 >= 5.0e-2);
        if (unstable_check_Q) { // energy is increasing
            std::cout << std::endl << std::endl << bold << red << "   Simulation is unstable" << std::endl;
            break;
        }
        energy = energy_n;
        std::cout << std::endl << std::endl;
