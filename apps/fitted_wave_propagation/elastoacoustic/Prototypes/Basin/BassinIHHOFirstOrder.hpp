...
void BassinIHHOFirstOrder(int argc, char **argv);
...
void BassinIHHOFirstOrder(int argc, char **argv){
    ...
    mesh_files.push_back("../../meshes/essai_tris_non_conformes.txt"); // l = 1
    mesh_files.push_back("../../meshes/quad_small.txt");               // l = 2
    mesh_files.push_back("../../meshes/tri_small.txt");                // l = 3
    mesh_files.push_back("../../meshes/strat.txt");                    // l = 4
    ...
    for (auto face_it = msh.faces_begin(); face_it != msh.faces_end(); face_it++) {
        ...
        if (materials[0] == 3) {
            if (materials[1] == 1 || materials[1] == 2) {
                ...
                // Ajouter la face parcourue a la structure interfaces 
                continue;
            }
        }
        if (materials[1] == 3) {
    ...
    // assembler.project_over_cells(msh, x_dof, v_fun_elastic, null_flux_fun, null_s_fun, null_fun);
    // assembler.project_over_faces(msh, x_dof, v_fun_elastic, null_s_fun);
    ...
    // typename mesh_type::point_type Acoustic_s1_pt(-450, 1200);
    ...
    //     postprocessor<mesh_type>::record_velocity_data_elasto_acoustic_four_fields(0, Acoustic_s1_pt_cell, msh, hho_di, assembler, x_dof, a_side_Q, Acoustic_sensor_1_log);
