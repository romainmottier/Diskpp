...
void AcousticIHHOFirstOrder(int argc, char **argv);
...
void AcousticIHHOFirstOrder(int argc, char **argv){
  ...
  assembler.project_over_cells(msh, x_dof, null_fun, null_flux_fun, vel_fun, null_fun);
  assembler.project_over_faces(msh, x_dof, null_fun, vel_fun);
  ...
  std::ofstream dissipation_file("dissipation_file.txt");
    ...
    std::ofstream Acoustic_sensor_1_log("Acoustic_s1_four_fields_h.csv");
    std::ofstream Acoustic_sensor_2_log("Acoustic_s2_four_fields_h.csv");
    std::ofstream Acoustic_sensor_3_log("Acoustic_s3_four_fields_h.csv");
    std::ofstream Acoustic_sensor_4_log("Acoustic_s4_four_fields_h.csv");
    bool e_side_Q = true;
    bool a_side_Q = false;
    typename mesh_type::point_type Acoustic_s1_pt(-0.10, 0.25);
    typename mesh_type::point_type Acoustic_s2_pt(-0.10, 0.00);
    typename mesh_type::point_type Acoustic_s3_pt(-0.10,-0.25);
    ...
    std::pair<typename mesh_type::point_type,size_t> Acoustic_s3_pt_cell  = std::make_pair(Acoustic_s3_pt, -1);
    std::cout << bold << cyan << "      " << "Acoustic sensor at (-0.1,0.25)" << reset << std::endl; 
    ...
    std::cout << bold << cyan << "      " << "Acoustic sensor at (-0.1,0.0)" << reset << std::endl; 
    postprocessor<mesh_type>::record_acoustic_data_elasto_acoustic_four_fields(0, Acoustic_s2_pt_cell, msh, hho_di, assembler, x_dof, a_side_Q, Acoustic_sensor_2_log);
    std::cout << bold << cyan << "      " << "Acoustic sensor at (-0.1,0.25)" << reset << std::endl; 
    postprocessor<mesh_type>::record_acoustic_data_elasto_acoustic_four_fields(0, Acoustic_s3_pt_cell, msh, hho_di, assembler, x_dof, a_side_Q, Acoustic_sensor_3_log);  
  ...
  for(size_t it = 1; it <= nt; it++) {
    ...
    postprocessor<mesh_type>::record_acoustic_data_elasto_acoustic_four_fields(it, Acoustic_s2_pt_cell, msh, hho_di, assembler, x_dof, e_side_Q, Acoustic_sensor_2_log);
    postprocessor<mesh_type>::record_acoustic_data_elasto_acoustic_four_fields(it, Acoustic_s3_pt_cell, msh, hho_di, assembler, x_dof, e_side_Q, Acoustic_sensor_3_log);
