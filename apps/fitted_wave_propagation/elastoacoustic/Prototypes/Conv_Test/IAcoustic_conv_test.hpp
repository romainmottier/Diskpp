...
void IAcoustic_conv_test(int argc, char **argv){
    ...
    std::cout << std::endl << bold << red << "   IMPLICIT ACOUSTIC CONV TEST" << std::endl;
    ...
    tc.tic();
    assembler.assemble(msh, null_fun, s_f_fun, false);
    tc.toc();
    ...
    filename << "A_implicit_l_" << sim_data.m_n_divs << "_n_" << sim_data.m_nt_divs << "_k_" << sim_data.m_k_degree << "_s_" << s << ".txt";
    ...
    filename_e << "A_energy_l_" << sim_data.m_n_divs << "_n_" << sim_data.m_nt_divs << "_k_" << sim_data.m_k_degree << "_s_" << s << "_discret_" << sim_data.m_hdg_stabilization_Q << ".txt";
    ...
    for(size_t it = 1; it <= nt; it++){
        ...
        tc.tic();
        {
            ...
            for (int i = 0; i < s; i++) {
                ...
                assembler.assemble_rhs(msh, null_fun, s_f_fun, false);
